...
class DCGAN(object):
  ...
  def discriminator(self, image, y=None, reuse=False):
    with tf.variable_scope("discriminator") as scope:
      ...
      if not self.y_dim:
        ...
        h4 = linear(tf.reshape(self.h3, [self.batch_size, -1]), 1, 'd_h4_lin')
        ...
        h1 = tf.reshape(h1, [self.batch_size, -1]) #flatten
        h1 = concat([h1, y], 1)
  ...
  def feature_match_layer(self, image, y=None, reuse=False):
      with tf.variable_scope("discriminator") as scope:
        if reuse:
          scope.reuse_variables()
  ...
  def get_test_data(self):
    ...
    batch = [get_image(name, input_hight=self.input_height, input_width = self.input_width, resize_height = self.output_height, resize_width = self.output_width, crop = self.crop, grayscale=self.grayscale) for name in sefl.test_data_names]
    if self.grayscale:
      batch_images = np.array(batch).astype(np.float32)[:,:,:,None]
    else:
      batch_images = np.array(batch).astype(np.float32)
  ...
  def anomaly_detector(self, ano_para=0.1, dis_loss='feature'):
    ...
    with variable_scope("anomaly_detector"):
      ...
      self.test_inputs = tf.placeholder(tf.float32, [None] + image_dims, name='test_images')
      test_inputs = self.test_inputs

      self.ano_z = tf.get_variable('ano_z', shape = [None, self.z_dim], dtype = tf.float32, 
        initializer = tf.random_uniform_initializer(minval=-1, maxval=1, dtype=tf.float32))

      self.ano_y = None

      self.ano_G = self.generator(self.ano_z, self.ano_y, reuse=True)

      self.res_loss = tf.reduce_mean(
        tf.reduce_sum(tf.abs(tf.subtract(test_inputs, self.ano_G)), axis=[1,2,3]))

      #Create Anomaly Score 
      if dis_loss = 'feature': # if discrimination loss with feature matching (same with paper)
        dis_f_z, dis_f_input = feature_match_layer(self.ano_G, self.ano_y, reuse=True), feature_match_layer(test_inputs, self.ano_y, reuse=True)
        self.dis_loss = tf.reduce_mean(
          tf.reduce_sum(tf.abs(tf.subtract(dis_f_z, dis_f_input)),axis=[1,2,3]))
      else: # if dis_loss with original generator's loss in  DCGAN
        test_D, test_D_logits_ = self.discriminator(ano_G, ano_y, reuse=True)
        self.dis_loss = tf.reduce_mean(
  ...
  def train_anomaly_detector(self, config, test_data, test_data_name):
    ...
    for epoch in range(config.test_epoch):
      ...
      print("Epcoh: [%2d] [%4d/%4d], anomaly score: %.8f, res loss: %.8f"\
        % (epoch, iter, self.test_batch_size, score, res_loss))

      if epoch==0 or np.mod(epoch, 50) == 49:
        samples = self.sess.run(self.ano_G)
        errors = samples-test_data

        imsave(samples[_iter], './{}/test_{}_{:02d}.png'.format(config.test_dir,  test_data_name, epoch))
        imsave(errors[_iter], './{}/test_error_{}_{:02d}.png'.format(config.test_dir, test_data_name,epoch))
